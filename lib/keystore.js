...
KeyStore.prototype.sign = function (hash, password, signingAddress) {
  hash = new Buffer(hash, 'hex');
  if (this.addresses.length === 0) {
    throw new Error('KeyStore.sign: No private keys in KeyStore.');
  }
  ...
  if (signingAddress === undefined) {
    ...
    if (this.encPrivKeys[signingAddress] === undefined) {
      throw new Error('KeyStore.sign: Address not found in KeyStore');
    }
  ...
  if (addrFromPrivKey !== address) {
    throw new Error('KeyStore.sign: Decrypting private key failed!');
  }
  var keyPair = ec.genKeyPair();
  keyPair._importPrivate(privKey, 'hex');
...
KeyStore.prototype.recoverAddress = function(msg, signature, recoveryParam) {
  var recoveredPub = ec.recoverPubKey(msg, signature, recoveryParam, 'hex');
  var pair = ec.keyFromPublic(recoveredPub);
  return this.addrFromPub(pair.getPublic(false, 'hex').slice(2));
};

KeyStore.prototype.addrFromPub = function(pubKey) {
  var pubKeyWordArray = CryptoJS.enc.Hex.parse(pubKey);
  var hash = CryptoJS.SHA3(pubKeyWordArray, { outputLength: 256 });
  var address = hash.toString(CryptoJS.enc.Hex).slice(24);
  return address;
